describe('Realizando requisições para a API', () => {
  ...
  context('GET /users/:userId', () => {
    ...
    it('Deve retornar um erro quando o usuário for inválido', () => {
      cy.request({
        ...
        failOnStatusCode: false, // não falha o teste antes que eu verifique
      }).then((response) => {
  ...
  context('Teste método PUT da API Usuários', () => {
    ...
    it('Retorna erro 404 para usuário inexistente', () => {
      ...
      cy.request({
        method: 'PUT',
        url: 'http://localhost:8000/users/c691fd15-dcd5-4f24-89d',
        body: usuario,
        failOnStatusCode: false,
      }).then((response) => {
        expect(response.status).to.eq(404);
  ...
  context('Interceptando solicitações de rede', () => {
    it('Deve fazer a inteceptação do POST users/login', () => {
  ...
  context('Realizando login via API', ()=> {
    it('Deve permitir o login do usuário Nelson Seguins', () => {
      cy.request({
        method: 'POST',
